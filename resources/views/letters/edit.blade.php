...
<form method="post" action="{{ route('letters.update', $letter['id']) }}">
    ...
    @csrf
    <div class="mb-6">
      <label for="letter_perihal" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">letter_perihal</label>
      <input type="text" id="letter_perihal" name="letter_perihal" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('letter_perihal') is-invalid @enderror" value="{{ old('letter_perihal', $letter->letter_perihal) }}" required autofocus>
    ...
    <div class="mb-4">
        ...
        @foreach($users as $guru)
            <div class="flex items-center">
                <input type="checkbox" id="recipients{{ $guru->id }}" name="recipients[]" value="{{ $guru->id }}" class="mr-2" {{ in_array($guru->id, $recipients) ? 'checked' : '' }}>
