  ...
  <body class="bg-light">
    <div class="d-flex justify-content-center flex-column" style="margin-top: 10%">
        ...
        <div class="d-block m-auto">
            ...
            <a href="{{ route('dashboard.page') }}" class="btn btn-info mt-3">Kembali</a>
        @elseif (auth()->check() && Auth::user()->role == 'guru')
            <a href="{{ route('dashboard.page') }}" class="btn btn-info mt-3">Kembali</a>
        @else
            <!-- Handle jika user belum terautentikasi atau tidak memiliki properti 'role' -->
            <p>User belum terautentikasi atau tidak memiliki role yang diharapkan.</p>
        @endif
        </div>
    </div>
