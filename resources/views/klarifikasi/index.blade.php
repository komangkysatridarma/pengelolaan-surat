...
<div class="flex items-center mt-2 space-x-2">
  ...
  <div class="flex-grow mx-0">
      <form action="{{ route('klaf.home') }}" method="GET">
          ...
          <div class="relative flex items-center">
              ...
              <input type="search" name="search_letter_type" id="default-search" class="block w-full p-3 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search Kode Surat or Klasifikasi surat...." value="{{request('search_letter_type')}}" required>
      ...
      <tbody>
        @foreach ($letter_types as $lt)
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray">
              ...
              <td class="px-6 py-4">
                {{ \App\Models\Letter::countUsageByType($lt->id) }}
            </td>
              <td class="px-6 py-4 flex flex-row">
                <a href="{{ route('klaf.detail', $lt['id']) }}" class="pr-1">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                  </svg>                  
                </a>
                <a href="{{ route('klaf.edit', $lt['id']) }}" class="pr-1">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                </svg>
                </a>
                <form action="{{ route('klaf.delete', $lt['id']) }}" method="post" class="d-inline">
                  @method('delete')
                  @csrf
